namespace Lunr.Globalization.fr
{
	public sealed class FrenchStemmer : StemmerBase
	{
		...
		{
			sbp.ket = sbp.cursor;
			var among_var = sbp.FindAmongBackwards(a_4, 43);
			if (among_var != 0) /* !! */
			{
				sbp.bra = sbp.cursor;
				switch (among_var)
				{
					...
					case 15:
						...
						if (sbp.InGroupingBackwards(g_v, 97, 251) && r_RV())
						{
		...
		private bool r_i_verb_suffix()
		{
			...
			sbp.ket = sbp.cursor;
			var among_var = sbp.FindAmongBackwards(a_5, 35);
			if (among_var == 0) /* !! */
			{
			...
			if (among_var == 1)
			{
				if (!sbp.OutGroupingBackwards(g_v, 97, 251))
				{
					sbp.limit_backward = v_1;
					return false;
				}
		...
		private bool r_verb_suffix()
		{
			...
			sbp.limit_backward = I_pV;
			sbp.ket = sbp.cursor;
			var among_var = sbp.FindAmongBackwards(a_6, 38);
			if (among_var == 0) /* !! */
			{
		...
		private void r_residual_suffix()
		{
			...
			if (sbp.EqualsSegmentBackwards(1, "s"))
			{
				...
				if (sbp.OutGroupingBackwards(g_keep_with_s, 97, 232))
				{
			...
			if (sbp.cursor >= I_pV)
			{
				...
				sbp.limit_backward = I_pV;
				sbp.ket = sbp.cursor;
				var among_var = sbp.FindAmongBackwards(a_7, 7);
				if (among_var != 0) /* !! */
				{
					sbp.bra = sbp.cursor;
					switch (among_var)
		...
		{
			var v_1 = sbp.limit - sbp.cursor;
			if (sbp.FindAmongBackwards(a_8, 5) != 0) /* !! */
			{
				sbp.cursor = sbp.limit - v_1;
				sbp.ket = sbp.cursor;
		...
		private void habr5()
		{
			...
			sbp.cursor = sbp.limit;
			sbp.ket = sbp.cursor;
			if (sbp.EqualsSegmentBackwards(1, "Y"))
			{
				sbp.bra = sbp.cursor;
				sbp.SliceFrom("i");
			}
